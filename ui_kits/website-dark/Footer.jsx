...
function Footer({ onNav }) {
  ...
  return (
    <footer style={{ background: '#080808', borderTop: '1px solid var(--border)', padding: '60px 32px 28px' }}>
...
function BookingModal({ open, preset, onClose, onConfirm }) {
  ...
  if (!open) return null;
  const input = { width: '100%', background: '#181818', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', fontFamily: 'var(--font-body)', fontSize: 14, color: '#FAFAFA', boxSizing: 'border-box', marginTop: 6 };
  const lbl = { fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--muted)' };
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(8,8,8,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 440, maxWidth: '100%', background: '#111', border: '1px solid var(--border-strong)', borderRadius: 20, padding: '32px 32px 28px', boxShadow: '0 30px 80px rgba(0,0,0,.7)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <SectionLabel>Book This Package</SectionLabel>
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 28, color: '#FAFAFA', margin: '12px 0 0' }}>{preset || 'Your Session'}</h3>
          </div>
...
function Toast({ show, text }) {
  return (
    <div style={{ position: 'fixed', right: 24, bottom: 24, zIndex: 400, transform: show ? 'translateY(0)' : 'translateY(140%)', opacity: show ? 1 : 0, transition: 'all .5s cubic-bezier(.16,1,.3,1)', background: '#181818', border: '1px solid var(--border-strong)', borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 16px 50px rgba(0,0,0,.6)', maxWidth: 340 }}>
      <span style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--gold-gradient)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#080808', flex: 'none' }}><Icon name="bell" size={18} color="#080808" /></span>
      <div>
        <div style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 14, color: '#FAFAFA' }}>Booking received!</div>
        <div style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, color: '#A0A0A0', marginTop: 2 }}>{text}</div>
      </div>
    </div>
  );
}
