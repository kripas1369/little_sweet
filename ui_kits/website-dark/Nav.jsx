...
function NavDropdown({ label, items, onPick }) {
  ...
  return (
    <div style={{ position: 'relative' }} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
      <button style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 15, color: open ? 'var(--gold)' : '#EDEDED', padding: '8px 0', transition: 'color .2s' }}>
        {label}<span style={{ fontSize: 9, transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}>▾</span>
      </button>
      {open && (
        <div style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', marginTop: 4, background: 'rgba(17,17,17,0.97)', backdropFilter: 'blur(20px)', border: '1px solid var(--border)', borderRadius: 14, padding: 10, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, width: 360, boxShadow: '0 24px 60px rgba(0,0,0,.6)', zIndex: 50 }}>
          {items.map((it) => (
            <div key={it} onClick={() => onPick && onPick(it)}
              style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color: '#C8C8C8', padding: '9px 12px', borderRadius: 8, cursor: 'pointer', transition: 'all .15s' }}
              onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(201,168,76,.08)'; e.currentTarget.style.color = 'var(--gold)'; }}
...
function NavLink({ children, onClick, active }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ background: 'none', border: 'none', cursor: 'pointer', position: 'relative', fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 15, color: (hover || active) ? 'var(--gold)' : '#EDEDED', padding: '8px 0', transition: 'color .2s' }}>
      {children}
...
function Navbar({ onBook, onNav, route, onPick }) {
  ...
  return (
    <React.Fragment>
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(8,8,8,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button className="nav-burger" onClick={() => setDrawer(true)} style={{ display: 'none', background: 'none', border: 'none', color: 'var(--gold)', cursor: 'pointer' }}><Icon name="menu" size={26} /></button>
        </div>
      </nav>

      {drawer && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(8,8,8,0.97)', backdropFilter: 'blur(20px)', padding: 32, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <Logo width={110} />
            <button onClick={() => setDrawer(false)} style={{ background: 'none', border: 'none', color: 'var(--gold)', cursor: 'pointer' }}><Icon name="x" size={28} /></button>
          </div>
          {['Home', 'About', 'Contact'].map((l) => (
            <div key={l} onClick={() => { onNav(l.toLowerCase()); setDrawer(false); }} style={{ fontFamily: 'var(--font-display)', fontSize: 30, color: '#FAFAFA', padding: '10px 0', cursor: 'pointer', borderBottom: '1px solid rgba(201,168,76,.12)' }}>{l}</div>
          ))}
