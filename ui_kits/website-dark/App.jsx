...
function LoginModal({ open, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(8,8,8,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 400, maxWidth: '100%', background: '#111', border: '1px solid var(--border-strong)', borderRadius: 20, padding: '34px 32px', textAlign: 'center' }}>
        <Logo width={120} />
        <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 26, color: '#FAFAFA', margin: '20px 0 6px' }}>Welcome Back</h3>
...
function App() {
  ...
  return (
    <div style={{ background: '#080808', minHeight: '100vh' }}>
      ...
      <Hero onExplore={() => { const el = document.getElementById('ceremonies'); el && window.scrollTo({ top: el.offsetTop - 70, behavior: 'smooth' }); }} />
      <SearchBar onSearch={(t) => openBooking(typeof t === 'string' ? t : '')} />
      <div id="ceremonies"><Ceremonies onBook={openBooking} /></div>
      <Business onBook={openBooking} />
      <WhyChoose />
      <HowItWorks />
      <Testimonials />
